...
public class UDPServer extends JFrame {
    ...
    private final Map<String, InetSocketAddress> clients = Collections.synchronizedMap(new HashMap<>());
    ...
    private void setupUI() {
        setTitle("Serveur + Client Chat UDP (" + name + ")");
        setSize(700, 500);
    ...
    private void startServer() {
        new Thread(() -> {
            try {
                ...
                appendText("🟢 Serveur UDP démarré sur le port " + PORT + "\n", Color.GREEN);
    ...
    private void handlePacket(DatagramPacket packet) {
        try {
            ...
            if (message.startsWith("REGISTER:")) {
                String clientName = message.substring(9);
                clients.put(clientName, clientAddress);
                broadcast("TEXT:ALL:Serveur:" + clientName + " vient de se connecter !");
                updateClientsList();
                appendText("🟢 " + clientName + " connecté\n", Color.GREEN);
            }
            else if (message.startsWith("TEXT:")) {
                handleTextMessage(message, clientAddress);
            }
            else if (message.startsWith("IMG:") || message.startsWith("AUDIO:") || message.startsWith("FILE:")) {
                handleFileMessage(message, packet);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void handleTextMessage(String message, InetSocketAddress senderAddress) {
        try {
            ...
            if (dest.equals("TOUS")) {
                broadcast("TEXT:ALL:" + sender + ":" + msg);
            } else {
                sendToClient(dest, "TEXT:PRIVATE:" + sender + ":" + msg);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void handleFileMessage(String message, DatagramPacket packet) {
        try {
            // Transmettre le paquet tel quel à tous les clients concernés
            ...
            if (dest.equals("TOUS")) {
                for (String clientName : clients.keySet()) {
                    if (!clientName.equals(sender)) {
                        sendDataToClient(clientName, packet.getData(), packet.getLength());
                    }
                }
            } else {
                sendDataToClient(dest, packet.getData(), packet.getLength());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void sendToClient(String clientName, String message) {
        try {
            InetSocketAddress address = clients.get(clientName);
            if (address != null) {
    ...
    private void sendDataToClient(String clientName, byte[] data, int length) {
        try {
            InetSocketAddress address = clients.get(clientName);
            if (address != null) {
                DatagramPacket packet = new DatagramPacket(data, length, address.getAddress(), address.getPort());
                serverSocket.send(packet);
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private void updateClientsList() {
        ...
        broadcast("LISTE:" + list.toString());
    }

    // ===========================
    // CLIENT PART (SERVER AS CLIENT)
    // ===========================
    ...
    private void sendFile(String type, File file) {
        ...
        try {
            ...
            DatagramPacket packet = new DatagramPacket(combinedData, combinedData.length, InetAddress.getByName("localhost"), PORT);
            clientSocket.send(packet);
